					...
					*/}
					<div className="bbs_faq">
						...
						}
						{
						 faqList.map((a,i) =>{
							 return (
								<Card faqList={faqList[i]} tabToggle={tabToggle}/>
							 )
						 })
						}
					</div>	
					{
						 faqList.map((re, index) =>{
							return (
								<>
								<button onClick={showHide}>{faqList[index].title}</button>
								{
									누른제목 === true 
...
function Card(props){
	return (
		<dl>
			<dt onClick={props.tabToggle}><span>{props.faqList.title}</span></dt>
			<dd>{props.faqList.content}</dd>
		</dl>
	)
  }
